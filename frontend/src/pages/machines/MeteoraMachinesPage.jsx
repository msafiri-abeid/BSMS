...
export default function MeteoraMachinesPage() {
  ...
  const { data: shopsData } = useQuery({ queryKey: ['shops-meteora-list'], queryFn: () => shopsAPI.list({ business_type: 'meteora' }).then(r => r.data.data) });
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = 'meteora-machines_' + dayjs().format('YYYY-MM-DD') + '.xlsx';
      a.click();
  ...
  const handleExportSelected = () => {
    ...
    const csv = [
      ['Slot Code', 'Location', 'Weekly Target', 'Debt', 'Status'].join(','),
      ...selected.map(r =>
        [r.slot_code, r.currentShop?.name || 'Office',
    ...
    a.download = `meteora-machines-selected-${dayjs().format('YYYY-MM-DD')}.csv`; a.click();
  };

  const handleAction = (key, r) => {
    if (key === 'view') navigate('/machines/meteora/' + r.id);
    if (key === 'edit') {
  ...
  const cols = [
    ...
    { title: 'Location', dataIndex: ['currentShop', 'name'], render: v => v || <Tag>Office</Tag>, width: 130 },
    {
      title: 'Weekly Target', dataIndex: 'effective_target_tzs', render: v =>
        v ? 'TZS ' + v.toLocaleString() : <span className="text-xs text-slate-400">—</span>, width: 120,
    },
    {
      title: 'Debt', dataIndex: 'outstanding_debt_tzs', render: v =>
        v > 0 ? <span className="font-semibold text-red-600">TZS {v.toLocaleString()}</span> : <span className="text-xs text-slate-300">—</span>, width: 110,
    },
  ...
  return (
    <div>
          ...
          <List
            ...
            renderItem={(r) => (
              <MobileCard
                record={r}
                fields={mobileFields}
                onClick={() => navigate('/machines/meteora/' + r.id)}
      ...
      <Modal title={<span className="text-sm font-bold text-slate-700">Register Meteora Machine</span>}
        open={registerOpen} onCancel={() => setRegisterOpen(false)}
        onOk={() => form.submit()} confirmLoading={createMutation.isPending}
        className="top-8">
        <Form form={form} layout="vertical" onFinish={(v) => createMutation.mutate({ ...v, manufacturer: 'Meteora' })} className="mt-4">
          ...
          <Form.Item name="credit_value_tzs" label={<span className="text-xs font-semibold text-slate-600">Credit Value (TZS)</span>} rules={[{ required: true }]} initialValue={200}>
