...
import { Table, Typography, App, DatePicker, Tag, Spin, List } from 'antd';
...
export default function TrialBalancePage() {
  ...
  const mobileFields = [
    { key: 'name', label: 'Account', render: (_, r) => <span className="font-semibold text-sm">{r.name}</span> },
    { key: 'type', label: 'Type', render: (_, r) => <Tag className="!text-[10px] capitalize">{r.type?.replace('_', ' ')}</Tag> },
    { key: 'debit', label: 'Debit', render: (_, r) => r.debit > 0 ? <span className="font-semibold text-emerald-600">{fmt(r.debit)}</span> : '—' },
    { key: 'credit', label: 'Credit', render: (_, r) => r.credit > 0 ? <span className="font-semibold text-red-600">{fmt(r.credit)}</span> : '—' },
  ];
