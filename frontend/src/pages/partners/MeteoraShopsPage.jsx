...
import { Table, Button, Modal, Form, Input, Select, Tag, App, Upload, Space, List, Empty } from 'antd';
...
export default function MeteoraShopsPage() {
  ...
  const onFinish = (values) => {
    ...
    fd.append('business_type', 'meteora');
  ...
  const handleExport = async () => {
    try {
      ...
      a.download = `meteora-shops-${dayjs().format('YYYY-MM-DD')}.xlsx`; a.click();
  ...
  const handleExportSelected = () => {
    ...
    const csv = [
      ['Name', 'Partner', 'Label', 'Status'].join(','),
      ...selected.map(r => [r.name, r.partner?.name, r.partner?.label, r.status].join(',')),
    ].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url;
    a.download = `meteora-shops-selected-${dayjs().format('YYYY-MM-DD')}.csv`; a.click();
  };

  const handleAction = (key, r) => {
    if (key === 'view') navigate(`/shops/meteora/${r.id}`);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <KpiCard title="Total Meteora Shops" value={rows.length} icon={Store} bgColor="bg-slate-50" iconColor="text-slate-600" />
          ...
          <List
            ...
            renderItem={(r) => (
              <MobileCard
                record={r}
                fields={mobileFields}
                onClick={() => navigate(`/shops/meteora/${r.id}`)}
